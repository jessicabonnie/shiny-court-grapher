...
CMS_MOT_FY15 <- filter(CMS, CASE.TYP =="MC", FYear> 2014, HEAR.RSLT %in% c("MO", "I")) %>%
...
CMS_MOT_FY15 <- CMS_MOT_FY15[complete.cases(CMS_MOT_FY15),]

CMS_MOT_FY15 <- spread(CMS_MOT_FY15,MOT_TYPE,count)

CMS_MOT_FY15[is.na(CMS_MOT_FY15)] <- 0

CMS_MOT_FY15$Total <- CMS_MOT_FY15$TYPE1 + CMS_MOT_FY15$TYPE2 + CMS_MOT_FY15$TYPE3 + CMS_MOT_FY15$TYPE4

names(CMS_MOT_FY15) <- c("Locality", "Direct", "New\nHearing", "Discharge\nInitial", "Discharge\nRecommitment", "Total")
CMS_MOT_FY15$Locality <- gsub("/","/\n",CMS_MOT_FY15$Locality)
CMS_MOT_FY15$Locality <- gsub(" \\(","\n\\(",CMS_MOT_FY15$Locality)
